...
$(document).ready(() => {
  ...
  const fetchAndDsiplayList = (apiUrl, listElementId) => {
  ...
  fetchAndDsiplayList("/api/top-picks", "favoritesList");
  fetchAndDsiplayList("/api/popular", "popularList");
  fetchAndDsiplayList("/api/suggested", "suggestedList");
  fetchAndDsiplayList("/api/recommended", "recommendedList");

  const favoritesData = [
    {
      imgSrc: "images/favorite/f1.jpg",
      title: "Insidious",
      ageRating: "16+",
      duration: "1h 43min",
      likes: 19,
  ...
  // upcoming contents section

  //  suggested section

  const suggestedSportsList = document.getElementById("suggestedList");
  suggestedSportsData.map((item) => {
    const li = document.createElement("li");
    li.className = "slide-item";
    li.innerHTML = `
      <div class="block-images position-relative">
        ...
        <div class="block-description">
          ...
          <div class="movie-time d-flex align-items-center my-2">
            <div class="badge badge-secondary p-1 mr-2">${item.badge}</div>
    ...
    `;
    suggestedSportsList.appendChild(li);
  });

  // Recommended For You

  const favoritesSliderList = document.getElementById("recommendedList");
  recommendedMoviesData.map((item) => {
    const li = document.createElement("li");
    li.className = "slide-item";
    li.innerHTML = `
    <div class="block-images position-relative">
    ...
    <div class="block-description">
      <h6 class="iq-title"><a href="#">${item.title}</a></h6>
      <div class="movie-time d-flex align-items-center my-2">
        <div class="badge badge-secondary p-1 mr-2">${item.ageRating}</div>
        <span class="text-white">${item.duration}</span>
      </div>
      <div class="hover-buttons">
        <span class="btn btn-hover iq-button">
          <i class="fa fa-play mr-1"></i> Play Now
        </span>
      </div>
    </div>
    <div class="block-social-info">
    ...
    `;
    favoritesSliderList.appendChild(li);
  });

  // Initialize favorites slider

  // Trending topten section
